    ...
    ctx.console.addEntry(
      process.env.KOISHI_BASE
        ...
        : process.env.KOISHI_ENV === "browser"
          ? [resolve(baseDir, "../client/index.ts")]
          : {
...
export const Config: Schema<Config> = Schema.object({
  root: Schema.path({
    ...
    .default("data/logs")
    .description("存放输出日志的本地目录。"),
  maxAge: Schema.natural()
    .description("日志文件保存的最大天数（天）。")
    .default(30),
  maxSize: Schema.natural()
    .description("单个日志文件的最大大小（字节）。")
    .default(1024 * 100),
  levels: Schema.dict(Schema.natural().default(3)).description(
    "指定模块的输出等级。<br>示例： 左侧写 `debug`，右侧写 `3`",
  ),
});
